...
This document defines how Codex should maintain a persistent knowledge layer for
the homelab platform. The goal is to compile useful knowledge once, keep it
current, and avoid rediscovering the same context from raw sources in every
session.
...
| --- | --- | --- |
| Raw sources | Source material such as repo files, approved Notion pages, Linear issues, read-only cluster reports, articles, screenshots, and incident notes | Read-only unless the source itself is the approved target of a separate edit gate |
| Compiled wiki | Human-readable summaries, system pages, architecture notes, runbooks, decision records, indexes, and synthesis pages | Codex may update after the relevant documentation gate is approved |
| Schema | `AGENTS.md`, this file, `docs/operating-workflow.md`, and `docs/documentation-strategy.md` | Update when the operating model changes |

The compiled wiki is not a replacement for operational source of truth. It is a
maintained navigation and synthesis layer over the repo, Notion, Linear, and
...
| System | Owns |
| --- | --- |
| GitHub | Exact commands, manifests, scripts, runbooks, operating rules, and knowledge-wiki schema |
| Notion | Browsable knowledge pages, architecture narrative, decisions, system summaries, learning notes, and human navigation |
| Linear | Execution status, priority, blockers, and accepted work items |

Do not create two full copies of the same truth. If GitHub owns an exact runbook,
Notion should summarize and link. If Notion owns a long-form explanation, GitHub
should link or keep only the operational summary needed near the code.
...
| `docs/knowledge-log.md` | Append-only chronological record of ingests, queries, and lint passes |
...
meaningful knowledge ingest, durable synthesis, or wiki lint pass.
...
1. Classify the source: repo, Notion, Linear, live read-only status, web source,
   image, incident note, or user-provided text.
...
7. Update the smallest useful set of GitHub or Notion pages.
...
3. Read the relevant GitHub docs, manifests, or Notion pages.
...
decision summaries, glossary entries, incident lessons, and runbook improvements.
...
- duplicated source-of-truth between GitHub and Notion
...
Linting should usually produce findings and a proposed edit gate. Do not silently
rewrite durable architecture, security, network, access, VM, or live-cluster
policy.
...
- `Pages touched:` files or Notion pages changed
...
- If a web source or old note makes a drifting claim, mark it as unverified until
  it is checked against the current authoritative source.
...
- use Notion search when working in the human knowledge layer
